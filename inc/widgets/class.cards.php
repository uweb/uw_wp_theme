...
class UW_Widget_Cards extends WP_Widget
{
  ...
  {
		parent::__construct( $id = 'uw-widget-cards', $name = 'UW Image Cards', $options = array( 'description' => 'Choose from several styles of cards', 'classname' => 'cards-widget' ) );
  ...
  function form( $instance )
  {
    ...
    // Variable to hold default value
    $radio_checked = empty($radio) ? 'checked="checked"' : checked( $radio , "default-card", $echo = false );
    ...
    <label for="">Default card (<a id="enchanced-preview" href="#">preview<span><img src="<?php echo get_template_directory_uri(); ?>/assets/images/widget-card-default.jpg" alt="" width="" height="" />
     ...
     <input id="kjhsdf" type="radio" name="<?php echo $this->get_field_name('radio_card'); ?>" value="default-card"  <?php echo $radio_checked; ?> /></label>
    </p>
     <p>
  ...
  function widget($args, $instance)
  {
    ...
    $the_image = empty($image) ? [''] : wp_get_attachment_image_src( $image, array(750, 750) );
    ?>
