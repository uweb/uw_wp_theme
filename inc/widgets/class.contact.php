...
class UW_Widget_Contact extends WP_Widget
{
  ...
  <div id="the-people" class="uw-contact-card-the-people">
    ...
    if (isset($instance['person_name'])) {
      ...
      if(is_array($remake_form_person)){
        ...
        foreach ($remake_form_person as $key => $values) {
            ...
            echo '<div class=\'peep\'><p class=\'person-name\'><input data-posttype="post" class="widefat wp-get-posts" id="" name="' . $this->get_field_name( 'person_name' ) . '[]' . '" type="text" value="' . $name . '" placeholder="Name"/></p>';
            echo '<p class=\'person-title\'><input data-posttype="post" class="widefat wp-get-posts" id="" name="' . $this->get_field_name( 'person_title' ) . '[]' . '" type="text" value="' . $title . '" placeholder="Title"/></p>';
            echo '<p class=\'person-phone\'><input data-posttype="post" class="widefat wp-get-posts" id="" name="' . $this->get_field_name( 'person_phone' ) . '[]' . '" type="text" value="' . $phone . '" placeholder="Phone number"/></p>';
            echo '<p class=\'person-email\'><input data-posttype="post" class="widefat wp-get-posts" id="" name="' . $this->get_field_name( 'person_email' ) . '[]' . '" type="text" value="' . $email . '" placeholder="Email"/></p><a class=\'remove uw-contact-card-remove button\'>Remove</a></div>';
    ...
    <a class="button button-primary uw-contact-card-add-person-button" href="#" id="person-button" style="margin: 10px 0;">Add person</a>
    ...
    <script type="text/template" id="redirect-template" class="uw-contact-card-redirect-template">
        ...
        <div class='peep'>
          ...
          <a class="remove uw-contact-card-remove button">Remove</a>
        </div>
    </script>
