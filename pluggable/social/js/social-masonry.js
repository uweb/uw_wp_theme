...
uw_golden.Instagram.Image = Backbone.Model.extend({});

uw_golden.Instagram.Backgrounds = Backbone.Collection.extend({

    model: uw_golden.Instagram.Image,
...
uw_golden.Instagram.View = Backbone.View.extend({
    last5: [-1, -1, -1, -1, -1], //contains indexes of the last 5 selected images
    offLimitsImg: [], //contains indexes of images relative to a selected tile
    isRendered: false,
    isVisible: false,
    ...
    makeListView: function makeListView() {
        ...
        for (var i = 0; i < 5; i++) {
            ...
            atts = {
                text: uw_golden.wrapHashtagsLinks(instagram.caption.text),
                link: instagram.link,
    ...
    rotateBackground: function rotateBackground() {
        ...
        tile.animate({ opacity: 0.1 }, 250, function () {
            tile.css('background-image', 'url(' + newImage + ')');
            tile.attr('data-img', dataImg);
            tile.animate({ opacity: 1 }, 250);
        });
    }
});

$(document).ready(function () {
    uw_golden.Instagram.backgrounds = new uw_golden.Instagram.Backgrounds();
    uw_golden.Instagram.view = new uw_golden.Instagram.View({ el: $('#social-wrapper'), collection: uw_golden.Instagram.backgrounds });
});