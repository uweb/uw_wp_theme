...
jQuery(function ($) {
	function setWidgetMargin($widget) {
		...
		if ($widget.hasClass('expanded')) {
			...
			if ($widget.attr('style')) $widget.data('original-style', $widget.attr('style'));
	...
	$("a.display-options").each(function () {
		...
		$displayOptionsButton.parent().removeClass('widget-control-noform').find('.spinner').remove().css('float', 'left').prependTo($displayOptionsButton.parent());
	});
	...
	$("div#widgets-right, form#customize-controls").on("click", "a.add-condition", function (e) {
		...
		if ($(this).hasClass('active')) $widget.find('input[name=widget-conditions-visible]').val('1');else $widget.find('input[name=widget-conditions-visible]').val('0');
	});
