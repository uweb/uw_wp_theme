...
UW.Image = Backbone.View.extend({
  RATIO: 0.8,
  template: '<div class="uw-overlay">' + '<div></div>' + '<div class="wrapper" style="width:<%= width %>px; margin-top:-<%= height/2 %>px; margin-left:-<%= width/2 %>px;">' + '<span class="close"> Close</span>' + '<img src="<%= src %>" alt="<%=alt %>" style="width:100%;" />' + '<p><%= caption %></p>' + '</div>' + '</div>',
  templateVideo: '<div class="uw-overlay">' + '<div></div>' + '<div class="wrapper" style="width:<%= width %>px; margin-top:-<%= height/2 %>px; margin-left:-<%= width/2 %>px;">' + '<span class="close"> Close</span>' + '<iframe width="<%= width %>" height="<%= height %>" src="<%= src %>" style="border:0" allowfullscreen="" title="<%= caption %>"></iframe>' + '<p><%= caption %></p>' + '<p><%= credit %></p>' + '</div>' + '</div>',
  events: {
    'click': function click(e) {
      e.preventDefault();
      this.attrs = this.getAttributes(e); // This just checks to see if the anchor has a source (some slideshows and plugins use blank anchors to do their work)
  ...
  fetchImage: function fetchImage(e) {
    $('<img src="' + this.attrs.src + '"/>').imagesLoaded(this.overlay);
    return false;
  },
  overlay: function overlay(images) {
    var videoLightbox = this.attrs.rel.indexOf("uw-lightbox-video") > -1 ? true : false; // todo make this quicker
