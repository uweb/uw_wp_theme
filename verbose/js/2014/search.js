...
UW.Search = Backbone.View.extend({
  ...
  searchbar: '<div class="container no-height" role="search">' + '<div class="center-block uw-search-wrapper">' + '<form class="uw-search" action="<%= UW.baseUrl %>">' + '<div class="search-form-wrapper">' + '<label class="screen-reader" for="uw-search-bar">Enter search text</label>' + '<input id="uw-search-bar" type="search" name="s" value="" autocomplete="off" placeholder="Search" />' + '</div>' + '<select id="mobile-search-select" class="visible-xs" aria-label="Search scope">' + '<option value="uw" selected>All the UW</option>' + '<option value="site">Current site</option>' + '</select>' + '<input type="submit" value="search" class="search" tabindex="0"/>' + '<fieldset style="margin: 0; padding: 0; border: 1px solid #ffffff;">' + '<legend id="uw-search-label">Search scope</legend>' + '<div id="search-labels" class="labels hidden-xs">' + '<label class="radio">' + '<input class="radiobtn" type="radio" name="search" value="uw" data-toggle="radio" checked />' + 'All the UW' + '</label>' + '<label class="radio">' + '<input class="radiobtn" type="radio" name="search" value="site" data-toggle="radio" />' + 'Current site' + '</label>' + '</div>' + '</fieldset>' + '</form>' + '</div>' + '</div>',
  // Default values
  defaults: {},
  // List of events
  ...
  toggleBlur: function toggleBlur() {
  ...
  handleKeyDown: function handleKeyDown(event) {
    switch (event.keyCode) {
      case UW.KEYCODES.TAB:
        if ($(event.target)[0] == $('input.search')[0] && !$(event)[0].shiftKey) {
          $('#search-labels').addClass('focused');
        }

        if ($(event.target)[0] != $('input.search')[0] && $('#search-labels').hasClass('focused')) {
          $('#search-labels').removeClass('focused');
        }

        if ($(event.target)[0] == $('input.radiobtn')[0] && !$(event)[0].shiftKey) {
  ...
  // todo: clean up
  toggleSearchFeature: function toggleSearchFeature(event) {
    var mobileSelect = $('#mobile-search-select.visible-xs');

    if ('none' == mobileSelect.css('display')) {
      var value = $(event.currentTarget).find('input').val();
      this.searchFeature = value;
    } else {
      var _value = $(event.currentTarget).val();

      this.searchFeature = _value;
    }
  },
  // Skip the search if it is hidden when tabbing through
  skipToContentIfHidden: function skipToContentIfHidden() {
    if (!this.toggle.settings.isOpen) {
      $('#primary').focus();
    }
  },
