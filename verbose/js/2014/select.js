...
UW.Select = Backbone.View.extend({
  ...
  isDisabled: function isDisabled(selected) {
    var value = this.$el.find('li').eq(selected).data().value;
    return this.$select.find('option[value="' + value + '"]').prop('disabled');
  },
  closeWithoutAnimating: function closeWithoutAnimating() {
    this.$el.removeClass('open');
    this.$el.children().removeClass('open');
  },
  // Animate the select menu to the proper menu item.
  animate: function animate() {
    this.scroll = this.$target.offset().top - this.$el.find('li').first().offset().top; //var current_top = this.$el.position().top;

    this.$el.children('ul').animate({
      scrollTop: this.scroll
    }, {
      queue: false,
      complete: this.removeOpenClass
  ...
  cloneSelectEvents: function cloneSelectEvents() {
    ...
    if (this.$select.hasClass('uw-select-wp')) window.location = UW.baseUrl + '?cat=' + value;
  },
  ...
  openMenuOnKeydown: function openMenuOnKeydown(e) {
    // If the user tabs while the menu is not open then continue the focus to
    // the next element on the page.
    if (e.keyCode == this.keyCodes.tab && !this.isOpen()) return true; // If the user clicks a certain set of keys then proceed with the necessary
    // actions.
  ...
  atEdge: function atEdge(direction) {
    return this.current === 0 && direction === 'up' || this.current === this.$el.find('li').length - 1 && direction === 'down';
  },
