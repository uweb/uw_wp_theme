...
UW.YouTube = {}; //the UW.YouTube.Collection object contains data in models that refer to youtube videos
...
UW.YouTube.Collection = Backbone.Collection.extend({
  ...
  setup_for_type: function setup_for_type(youtube_id) {
  ...
  make_view: function make_view(type) {
    this.view = new UW.YouTube.CollectionView({
      collection: this
    });
  }
...
UW.YouTube.CollectionView = Backbone.View.extend({
  ...
  add_iFrame_api: function add_iFrame_api() {
  ...
  add_iFrame_function: function add_iFrame_function() {
    window.onYouTubeIframeAPIReady = function () {
      for (var i = 0, length = UW.youtube.length; i < length; i++) {
        var collection = UW.youtube[i],
            player_vars = {}; // if the collection desires no youtube branding, set these parameters
  ...
  add_playlist_section: function add_playlist_section() {
  ...
  check_all_ready: function check_all_ready() {
  ...
  preview_clicked: function preview_clicked(event) {
...
UW.YouTube.Video = Backbone.Model.extend({
  initialize: function initialize() {
    if (this.get('resourceId')) {
      this.view = new UW.YouTube.VideoView({
        model: this
      });
    }
  }
...
UW.YouTube.VideoView = Backbone.View.extend({
  ...
  render: function render() {
    this.model.collection.view.$el.attr('aria-label', 'video: ' + this.model.get('title')); // var item = this.model.toJSON();
...
UW.YouTube.PlaylistItem = Backbone.Model.extend({
  ...
  initialize: function initialize() {
    if (this.get('resourceId')) {
      this.view = new UW.YouTube.PlaylistItemView({
        model: this
      });
    }
  }
